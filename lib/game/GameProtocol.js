...
util.inherits(GameProtocol, Protocol);

_.extend(GameProtocol, {
...
_.extend(GameProtocol.prototype, {
	encodeSlotInfo: function (slots, /*uint32*/ randomSeed,/*uchar*/layoutStyle, /*uchar*/playerSlots) {
		var slotInfo = [
			[slots.length]
		];
		...
		return Bytes.Array(slotInfo);
	},

	SEND_W3GS_PING_FROM_HOST: function () {
	},

	SEND_W3GS_SLOTINFOJOIN: function () {
	},

	SEND_W3GS_REJECTJOIN: function (/* uint32 */ reason) {
		var packet = [];
		...
		return this.assignLength(Bytes.Array(packet));
	},

	SEND_W3GS_PLAYERINFO: function () {
	},

	SEND_W3GS_PLAYERLEAVE_OTHERS: function () {
	},

	SEND_W3GS_GAMELOADED_OTHERS: function () {
	},

	SEND_W3GS_SLOTINFO: function () {
	},

	SEND_W3GS_COUNTDOWN_START: function () {
	},

	SEND_W3GS_COUNTDOWN_END: function () {
	},

	SEND_W3GS_INCOMING_ACTION: function () {
	},

	SEND_W3GS_CHAT_FROM_HOST: function () {
	},

	SEND_W3GS_START_LAG: function () {
	},

	SEND_W3GS_STOP_LAG: function () {
	},

	SEND_W3GS_SEARCHGAME: function () {
	},

	SEND_W3GS_GAMEINFO: function (TFT,
	                              war3Version,
	                              mapGameType,
	                              mapFlags,
	                              mapWidth,
...
00 01 03 49 07 01 01 AD 01 C1 AD 01 6D FB CD 3B 4D 8B 61 71 73 5D 47 73 6F 85 7B 65 6F 55 69 73 6F A5 6F 65 5D 29 31 33 29 2F 45 6D 65 73 61 6D 65 A7 47 61 73 65 65 6F 73 8D 2F 77 33 79 01 57 61 39 73 6D 6F 63 6B 01 01
		...
		if (mapGameType.length === 4 &&
			...
			var statArray = [].concat(
				...
				mapHeight.toJSON(),
				mapCRC.toJSON(),
				Bytes.String(mapPath),
				Bytes.String(hostName),
				[0]
			);
			...
			var bytes = this.assignLength(Bytes.Array(packet));
	...
	SEND_W3GS_CREATEGAME: function () {
	},

	SEND_W3GS_REFRESHGAME: function () {
	},

	SEND_W3GS_DECREATEGAME: function () {
	},

	RECEIVE_W3GS_REQJOIN: function (buffer) {
		// 2 bytes					-> Header
		// 2 bytes					-> Length
	...
	RECEIVE_W3GS_LEAVEGAME: function () {
	},

	RECEIVE_W3GS_GAMELOADED_SELF: function () {
	},

	RECEIVE_W3GS_OUTGOING_ACTION: function () {
	},

	RECEIVE_W3GS_OUTGOING_KEEPALIVE: function () {
	},

	RECEIVE_W3GS_CHAT_TO_HOST: function () {
	},

	RECEIVE_W3GS_SEARCHGAME: function () {
	},

	RECEIVE_W3GS_MAPSIZE: function () {
	},

	RECEIVE_W3GS_MAPPARTOK: function () {
	},

	RECEIVE_W3GS_PONG_TO_HOST: function () {
	}
});
 ...
 * @constructor
 */
function IncomingJoinPlayer(hostCounter, entryKey, name, internalIPBuffer) {
	this.hostCounter = hostCounter;
	this.entryKey = entryKey;
	this.name = name;
	this.internalIPBuffer = internalIPBuffer;
}
