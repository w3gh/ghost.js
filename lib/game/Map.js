...
import { ByteExtract } from './../Bytes';
import GameSlot from './GameSlot';

export default class Map {
	constructor(ghost, path) {
		this.ghost = ghost;
		this.gameFlags = 0;
		this.slots = [];
	...
	loadDefaultMap() {
		...
		this.mapSize = ByteExtract('174 221 4 0', 4);
		this.mapInfo = ByteExtract('251 57 68 98', 4);
		this.mapCRC = ByteExtract('108 250 204 59', 4);
		this.mapSHA1 = ByteExtract('35 81 104 182 223 63 204 215 1 17 87 234 220 66 3 185 82 99 6 13', 20);
		...
		this.mapWidth = ByteExtract('172 0', 2);
		this.mapHeight = ByteExtract('172 0', 2);

		this.mapNumPlayer = 12;
		this.mapNumTeams = 1;

		const slots = [
			[0, 255, GameSlot.SLOTSTATUS_OPEN, 0, 0, 0, GameSlot.SLOTRACE_RANDOM | GameSlot.SLOTRACE_SELECTABLE],
			[0, 255, GameSlot.SLOTSTATUS_OPEN, 0, 1, 1, GameSlot.SLOTRACE_RANDOM | GameSlot.SLOTRACE_SELECTABLE],
			[0, 255, GameSlot.SLOTSTATUS_OPEN, 0, 2, 2, GameSlot.SLOTRACE_RANDOM | GameSlot.SLOTRACE_SELECTABLE],
			[0, 255, GameSlot.SLOTSTATUS_OPEN, 0, 3, 3, GameSlot.SLOTRACE_RANDOM | GameSlot.SLOTRACE_SELECTABLE],
			[0, 255, GameSlot.SLOTSTATUS_OPEN, 0, 4, 4, GameSlot.SLOTRACE_RANDOM | GameSlot.SLOTRACE_SELECTABLE],
			[0, 255, GameSlot.SLOTSTATUS_OPEN, 0, 5, 5, GameSlot.SLOTRACE_RANDOM | GameSlot.SLOTRACE_SELECTABLE],
			[0, 255, GameSlot.SLOTSTATUS_OPEN, 0, 6, 6, GameSlot.SLOTRACE_RANDOM | GameSlot.SLOTRACE_SELECTABLE],
			[0, 255, GameSlot.SLOTSTATUS_OPEN, 0, 7, 7, GameSlot.SLOTRACE_RANDOM | GameSlot.SLOTRACE_SELECTABLE],
			[0, 255, GameSlot.SLOTSTATUS_OPEN, 0, 8, 8, GameSlot.SLOTRACE_RANDOM | GameSlot.SLOTRACE_SELECTABLE],
			[0, 255, GameSlot.SLOTSTATUS_OPEN, 0, 9, 9, GameSlot.SLOTRACE_RANDOM | GameSlot.SLOTRACE_SELECTABLE],
			[0, 255, GameSlot.SLOTSTATUS_OPEN, 0, 10, 10, GameSlot.SLOTRACE_RANDOM | GameSlot.SLOTRACE_SELECTABLE],
			[0, 255, GameSlot.SLOTSTATUS_OPEN, 0, 11, 11, GameSlot.SLOTRACE_RANDOM | GameSlot.SLOTRACE_SELECTABLE]
		];
	...
	load(filename) {
		let mapCfgPath = this.ghost.mapCfgPath;

		if (!mapCfgPath) {
			return false;
		}

		let filepath = path.join(mapCfgPath, filename + '.json');
		...
		this.mapSize = Array.isArray(file.size) ? new Buffer(file.size) : ByteExtract(file.size, 4);
		this.mapInfo = Array.isArray(file.info) ? new Buffer(file.info) : ByteExtract(file.info, 4);
		this.mapCRC = Array.isArray(file.crc) ? new Buffer(file.crc) : ByteExtract(file.crc, 4);
		this.mapSHA1 = Array.isArray(file.sha1) ? new Buffer(file.sha1) : ByteExtract(file.sha1, 20);
		...
		this.mapWidth = Array.isArray(file.width) ? new Buffer(file.width) : ByteExtract(file.width, 2);
		this.mapHeight = Array.isArray(file.height) ? new Buffer(file.height) : ByteExtract(file.height, 2);

		this.mapNumPlayer = file.players;
		this.mapNumTeams = file.teams;

		if (Array.isArray(file.slots)) {
			file.slots.forEach((slot) => {
				this.slots.push(new GameSlot(...slot));
			});
		}
	}
