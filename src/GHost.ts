...
export class GHost extends Bot {
  ...
  constructor(cfg: Config) {
    ...
    this.bnet = new BNetCollection(
      this.cfg,
      this.TFT,
      this.hostPort,
      new GhostBNetSIDHandler(),
      new GhostBNetSIDReceiver()
    );
    ...
    this.on(Bot.EVENT_UPDATE, this.onUpdate).on(Bot.EVENT_EXIT, this.onExit);
  }
  ...
  onUpdate = () => {
    ...
    if (this.exitingNice) {
      if (!this.bnet.isEmpty()) {
        info(
          "deleting all battle.net connections in preparation for exiting nicely"
        );
  ...
  protected extractScripts() {
    ...
    MPQ.open(patchPath, null, (mpq) => {
      ...
      if (mpq.files.contains(commonJPath)) {
        ...
        info(
          `extracting Scripts\\common.j from MPQ file to '${this.mapConfigsPath}/common.j'`
        );
      }
      ...
      if (mpq.files.contains(blizzardJPath)) {
        ...
        info(
          `extracting Scripts\\blizzard.j from MPQ file to '${this.mapConfigsPath}/blizzard.j'`
        );
      }
    });
  }
  ...
  protected configure() {
    ...
    this.mapConfigsPath = this.toAbsolutePath(
      config.item("bot.mapcfgpath", "./maps")
    );
