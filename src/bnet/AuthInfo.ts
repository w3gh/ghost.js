...
export class AuthInfo implements IAuthInfo {
  ...
  handle(conn: BNetConnection) {
    let { exeInfo, exeVersion } = BNCSUtil.getExeInfo(
      conn.war3exePath,
    ...
    if (conn.exeVersion.length) {
      ...
      info(
        `[${
          conn.alias
        }] using custom exe version custom.exeversion ${JSON.stringify(
          exeVersion.toJSON().data
        )}`
      );
    } else {
      ...
      info(
        `[${conn.alias}] using exe version ${JSON.stringify(
          exeVersion.toJSON().data
        )}`
      );
    }
    ...
    if (conn.exeVersionHash.length) {
      ...
      info(
        `[${
          conn.alias
        }] using custom exe version hash custom.exeversionhash ${JSON.stringify(
          exeVersionHash.toJSON().data
        )}`
      );
    } else {
      ...
      info(
        `[${conn.alias}] using exe version hash ${JSON.stringify(
          exeVersionHash.toJSON().data
        )}`
      );
    }
    ...
    if (conn.TFT) {
      ...
      info(
        `[${conn.alias}] attempting to auth as "Warcraft III: The Frozen Throne"`
      );
    } else {
      info(
        `[${conn.alias}] attempting to auth as "Warcraft III: Reign of Chaos"`
      );
    }
