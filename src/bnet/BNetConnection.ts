 ...
 * @constructor
 */
export class BNetConnection
  extends EventEmitter
  implements IBNetConnection, IBNet
{
  ...
  public readonly clientToken: Buffer = Buffer.from(
    "\xdc\x01\xcb\x07",
    "binary"
  );
  ...
  constructor(
    ...
    info(
      `found battle.net connection [#${this.id}] for server [${this.server}]`
    );
  ...
  configure(config: Config) {
    ...
    this.maxMessageLength = Number(
      config.item("custom.maxmessagelength", "200")
    );
  ...
  configureSocket() {
    this.socket
      ...
      .on("error", (err) => {
        error(
          `[${this.alias}] disconnected from battle.net due to socket error ${err}`
        );
  ...
  extractPackets() {
    ...
    while (this.incomingBuffer.length >= 4) {
      ...
      if (!this.protocol.haveHeader(buffer)) {
        error(
          "received invalid packet from battle.net (bad header constant), disconnecting"
        );
        this.disconnect();
        return;
      }
      ...
      if (buffer.length >= bytesLength) {
        ...
        // still waiting for rest of the packet
        return;
      }
    }
  }
  ...
  processPackets() {
    while (this.incomingPackets.length) {
      ...
      if (packet.type === BNET_HEADER_CONSTANT) {
        ...
        handler &&
          receiver &&
          handler(this, this.protocol, receiver(packet.buffer));
      }
    }
  }
  ...
  update() {
    ...
    if (this.connected) {
      ...
      if (this.lastOutPacketSize < 10) this.waitTicks = 1300;
      else if (this.lastOutPacketSize < 30) this.waitTicks = 3400;
      else if (this.lastOutPacketSize < 50) this.waitTicks = 3600;
      else if (this.lastOutPacketSize < 100) this.waitTicks = 3900;
      else this.waitTicks = 5500;

      // add on frequency delay
      this.waitTicks += this.frequencyDelayTimes * 60;

      if (
        this.outPackets.length &&
        getTicks() - this.lastOutPacketTicks >= this.waitTicks
      ) {
        if (this.outPackets.length > 7) {
          info(
            `[${this.alias}] packet queue warning - there are ${this.outPackets.length} packets waiting to be sent`
          );
        }
        ...
        if (
          this.frequencyDelayTimes >= 100 ||
          getTicks() > this.lastOutPacketTicks + this.waitTicks + 500
        )
          this.frequencyDelayTimes = 0;
        else this.frequencyDelayTimes++;
    ...
    if (!this.connecting && !this.connected && this.firstConnect) {
      info(
        `[${this.alias}] connecting to server [${this.server}] on port 6112`
      );
      ...
      if (!this.connect()) {
        info(
          `[${this.alias}] failed to connect to server [${this.server}] on port 6112`
        );
      }
    }
  ...
  queueChatCommand(command: string, silent = false) {
    ...
    if (this._loggedIn) {
      if (
        this._passwordHashType === "pvpgn" &&
        command.length > this.maxMessageLength
      ) {
      ...
      if (this.outPackets.length > 10) {
        info(
          `[${this.alias}] attempted to queue chat command [${command}] but there are too many (${this.outPackets.length}) packets queued, discarding`
        );
      } else {
  ...
  queueGetGameList(gameName = "", numGames = 1) {
    if (this._loggedIn) {
      if (this.outPackets.length > 10) {
        info(
          `[${this.alias}] attempted to queue games list but there are too many (${this.outPackets.length}) packets queued, discarding`
        );
      } else {
        this.outPackets.push(
          this.protocol.SEND_SID_GETADVLISTEX(gameName, numGames)
        );
      }
    }
  }
