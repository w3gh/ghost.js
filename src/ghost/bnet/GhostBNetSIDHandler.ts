...
export class GhostBNetSIDHandler implements IBNetSIDHandler {
  ...
  [BNetSID.SID_AUTH_INFO](
    bnet: IBNetConnection,
    protocol: IBNetProtocol,
    authInfo: IAuthInfo
  ) {
  ...
  [BNetSID.SID_AUTH_CHECK](
    ...
    if (auth.isValid()) {
      ...
      bnet.sendPackets(
        protocol.SEND_SID_AUTH_ACCOUNTLOGON(clientPublicKey, bnet.username)
      );
    } else {
      info(`[${bnet.alias}] auth is invalid`);
      bnet.disconnect();
    }
  }
  ...
  [BNetSID.SID_AUTH_ACCOUNTLOGON](
    ...
    if (bnet.passwordHashType === "pvpgn") {
      ...
      info(
        `[${bnet.alias}] using battle.net logon type (for official battle.net servers only)`
      );
  ...
  [BNetSID.SID_CHATEVENT](
    bnet: IBNetConnection,
    protocol: IBNetProtocol,
    e: IIncomingChatEvent
  ) {
  ...
  [BNetSID.SID_CLANMEMBERSTATUSCHANGE](
    bnet: IBNetConnection,
    protocol: IBNetProtocol
  ) {
